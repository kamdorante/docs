---
title: Gestión de Ventas CRM
category: Documentation
star: 9
sticky: 9
article: false
---

# Gestión de Ventas CRM

Esta sección contiene las ventanas de gestión de relaciones con clientes (CRM), incluyendo el seguimiento de oportunidades de venta, prospectos, clientes y contactos.

## Ventanas

### Panel de Ventas
- [Panel del Agente Comercial](./sales-rep-dashboard)

### Gestión de Prospectos y Oportunidades
- [Prospecto](./lead)
- [Oportunidad de Venta](./sales-opportunity)

### Gestión de Clientes y Contactos
- [Cliente](./customer)
- [Contacto](./contact)

### Configuración
- [Etapa de Venta](./sales-stage)

## Consultas Inteligentes
- [Generar OC para Instalación de Producto](./generate-po-for-product-installation)

## Configuración
- [Servicios de Instalación en Productos de Arrendamiento](./rental-product-installation-services)
