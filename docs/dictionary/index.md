---
title: Diccionario de Aplicación
category: Documentation
star: 9
sticky: 9
article: false
---

# Diccionario de Aplicación

Esta sección contiene la documentación generada automáticamente desde el diccionario de datos de Solop ERP. Aquí encontrará la referencia completa de ventanas, procesos, reportes y navegadores inteligentes organizados por módulo funcional.

## Módulos

### Gestión Comercial

- [Gestión de Ventas](sales-management/)
- [Gestión de Compras](purchase-management/)
- [Gestión de Relaciones con Clientes](customer-relationship-management/)

### Gestión Financiera

- [Gestión de Saldos Pendientes](balance-management/)
- [Gestión Financiera](financial-management/)
- [Gestión Contable](accounting-management/)

### Gestión de Operaciones

- [Gestión de Materiales](material-management/)
- [Gestión de Producción](production-management/)
- [Gestión de Distribución](distribution-management/)
- [Gestión de Devoluciones](return-management/)

### Gestión de Recursos

- [Gestión Humana](human-management/)
- [Gestión de Activos](asset-management/)

### Módulos Especializados

- [Facturación Electrónica](electronic-billing/)
- [Verticales](verticals/)
- [Dispositivos](devices/)
- [Reglas Básicas y Configuración](basic-rules/)
